...
{
    print
        "--inXML is required with xml file which should be in TargetInstance format xml with mrw, fapia and non-fapi attributes to generate dts file\n";
    exit 1;
}
if ( $dtsFilename eq "" )
{
...
sub prepareDeviceTreeHierarchy
{
    ...
    foreach my $targetInstanceID (@sortedKeys)
    {
        ...
        {
            print
                "CRITICAL: \"PHYS_PATH\" attribute is not found for TargetInstance Target : \"$targetInstanceID\". So Ignoring\n"
                if isVerboseReq('C');
            next;
        }
...
sub handleArrayType
{
    ...
    setDTSFormatValueForSimpleAttr( $AttrID, $simpleType->DataType . "_" . $simpleType->subType,
        \@arrayValues, $eleCnt );
}
...
sub handleEnumType
{
    ...
    my $enumVal =
        $enumName ne "0" ? getEnumVal( \@{ $simpleType->enumDefinition->enumeratorList }, $enumName ) : $enumName;
    $enumVal = getEnumVal( \@{ $simpleType->enumDefinition->enumeratorList }, "" ) if $enumVal eq "";
...
sub getEnumVal
{
    ...
    if ( $enumName ne "" )
    {
        ...
        if (    ( substr( $enumName, 0, 2 ) eq "0x" )
            and ( substr( $enumeratorList[0][0], 0, 2 ) ne "0x" ) )
        {
...
sub getBinaryFormatForPath
{
    ...
    {
        print
            "CRITICAL: The max path element size is $configpathElementsSize but the given path element size in PHYS_DEV_PATH attribute value[$phyOrAffpath] is $pathElementsSize\n";
        return "";
    }
    ...
    foreach my $pathEle (@pathElements)
    {
        ...
        {
            print
                "CRITICAL: The given path element type[$pathEleFields[0]] for path[$path] is not found in TYPE attibute enum list.\n"
                if isVerboseReq('C');
            return "";
        }
    }
