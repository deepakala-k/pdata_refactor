...
sub prepareDeviceTreeHierarchy
{
    ...
    foreach my $MRWTargetID ( @sortedKeys)
    {
        ...
        print "deepa physPath :: $physPath\n";

        my $finalPath = $physPath;
        print "deepa 1111 finalPath::$finalPath\n";
        my $pervasivePath;
        ...
        if (
            exists $mrwTargetList{$MRWTargetID}->{targetAttrList}->{PARENT_PERVASIVE}
            && defined $mrwTargetList{$MRWTargetID}->{targetAttrList}->{PARENT_PERVASIVE})
        {
            # E.g. :
            ...
            print "deepa 2222 finalPath::$finalPath\n";
        }
        ...
        if ( exists ${$mrwTargetList{$MRWTargetID}->targetAttrList}{'I2C_PARENT_PHYS_PATH'} )
        {
            ...
            print "deepa 3333 finalPath::$finalPath\n";
        }
        ...
        if( index($mrwTargetList{$MRWTargetID}->targetType, "unit-fc-") != -1 )
        {
            ...
            print "deepa 4444 finalPath::$finalPath\n";
        }

        # Getting path value alone by ignoring "physical:"
        my $printTargetType = $mrwTargetList{$MRWTargetID}->targetType;
        print ("deepa foreach $printTargetType :::: $MRWTargetID -----------------\n");
        print "deepa:: $finalPath\n";
