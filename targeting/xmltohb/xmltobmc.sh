...
EKB_CUSTOMIZED_ATTRS_XML_BMC="bmc_customized_ekb_attrs.xml"
...
echo "Running: $COMMON_XMLTOHB_REL_PATH/$XMLTOHB_MERGE_SCRIPT $XMLTOHB_SRC_ATTRIBUTE_SOURCES > 
...
echo "Running: $COMMON_XMLTOHB_REL_PATH/$XMLTOHB_TARGET_MERGE_SCRIPT 
...
echo "Running: $script --ekbXmlFile=${GENDIR}/${XMLTOHB_EKB_ATTRIBUTE_TYPES} 
...
if [[ ! -f "$XMLTOHB_RAN_INDICATION" ]]; then
  ...
  "$COMMON_XMLTOHB_REL_PATH/${XMLTOHB_COMPILER_SCRIPT}" \
    ...
    --filter-attr-file="${TARGETING_XMLTOHB_REL_PATH}/$filter_attr" \
    --build-bmc \
...
SYSTEMS_MRW_XML=/gsa/ausgsa-p10/03/indiateam04/deepa/Projects/dts/rainier-xml/Rainier-2U-MRW.xml
            echo "Processing system xml " $(basename "$SYSTEMS_MRW_XML")
...
./filter_out_unwanted_attributes.pl --mrw-xml Rainier-2U-MRW_bmc_mrw.xml --tgt-xml target_types_src.xml --tgt-xml target_types_src.xml --attr-filter-file ../../xmltohb/filter_AttributesList.lsv --tgt-filter-file ../../xmltohb/filter_TargetsList.lsv
