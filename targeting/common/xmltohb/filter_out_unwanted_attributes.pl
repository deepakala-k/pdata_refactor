...
GetOptions(
    "tgt-xml=s"    => \@tgt_files,
    "mrw-xml=s"    => \$mrw_file,
    "attr-filter-file=s"=> \$attr_filter_file,
    "tgt-filter-file=s"=> \$target_filter_file,
    "help"         => \$help,
    "verbose|v"      => \$verbose,
);
...
sub usage
{
    ...
    print "     --verbose (or -v)\n";
    print "     --attr-filter-file [filter-file] (optional)";
    print "     --tgt-filter-file [filter-file] (optional)";
    exit (-1);
}
